...
Read this when the deliverable is a lifecycle diagnosis or retention proposals: the value model, the return event, the stage an intervention targets, the leak window a symptom belongs to. Benchmark levels are in `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/benchmarks.md`, metric conventions in `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/metric-definitions.md`, test design in `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/experiments.md` — this file carries none of them.
...
Prefer a **critical-event** definition wherever opening the app is not evidence of value — journaling, learning, companion, fortune, tools. The critical-event definition and the Amplitude onboarding-action figure behind it: `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/metric-definitions.md`. This **breaks comparability with vendor benchmarks on purpose**: say so, and stop quoting benchmarks at that product.

A return event means nothing without the other three axes — return rule, day boundary, denominator (`${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/metric-definitions.md`, read before quoting or comparing any number). Platform branch: PC/console report DAU/MAU and playtime, not day-N (`${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/genre-profiles.md`).
...
At-risk keys to the player's own median inter-session gap, not a global threshold — a weekly player is not lapsing on day 3. Signal→diagnostic mapping and win-back by lapse cause: `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/churn-and-winback.md`.

## Leak diagnosis by window

**Rule out three non-leaks first.** (1) A convention artifact — D1 is the convention-sensitive metric, and a calendar-vs-rolling-24h toggle moves it more than most product work does (`${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/metric-definitions.md`). (2) Expected post-launch decay — NCSOFT called Aion2's decline "along the expected decay curve" and pre-scheduled a six-month anniversary beat (FETV concall coverage, 2026). (3) Finite or binge-shaped products, where low DAU/MAU with long sessions is the design.
...
- Season and banner artifacts: `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/liveops-cadence.md`. Genre cadence units: `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/genre-profiles.md`.
...
- **Social without a meta layer produces spikes, not D30.** Co-op hits without meta-progression fall well short of meta-carrying comparators on D30; the levels, the titles behind them and why the comparison is confounded: `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/genre-profiles.md`.
- **"Social features raise retention" is not a claim to make.** In a Cox survival analysis of League of Legends players, connectedness lowered churn overall **but** playing with teammates was associated with lower win rates, which independently raise churn (Kim, Kim, Kim & Li, 2026 — `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/churn-and-winback.md`).
...
- Win-back reactivation and re-dormancy baselines (`${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/churn-and-winback.md`); login-calendar effect sizes, pass completion and repurchase rates (`${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/liveops-cadence.md`).
- Any retention **level** — D1/D7/D30, genre medians, percentile bands: `${CLAUDE_SKILL_DIR}/../retention-strategy-designer/references/benchmarks.md` only, with its `## Do not quote` list. Never average across rows.
