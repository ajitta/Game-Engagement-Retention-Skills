...
Read this whenever a proposal names a mechanic in one of the six domains below; `${CLAUDE_SKILL_DIR}/references/ethics-tiers.md` owns the tiers, procedure and refusal template, this file owns the rows. Every entry states its tier and carries a spec a reviewer can run against a build — no mechanic here is refusable by name alone.

**How to read a row.** Every domain runs in one order: **Forbidden (T1/T2a) → Compliant spec (T2b–T3) → Preference (T4, the author's stance)**. A row title that names a canonical mechanic family ends with that family's slug in backticks, and the card's `T<n> <family> ✓` bullet prints exactly that slug, nothing else; the compliant-spec index in `${CLAUDE_SKILL_DIR}/references/ethics-tiers.md` maps every slug to its tier and its owning section. A block with no slug is domain-scope (a forbidden list, a per-domain spec, a T4 stance) and never emits a family bullet.
...
**Forbidden (T1/T2a).** None as a mechanic — the statutes that bind this domain (개인정보 보호법 §23 and under-14 guardian consent, AI Basic Act Art. 31, 정보통신망법 §50) appear inside the T3 spec below and are T1 wherever they apply.
...
**Spec — T3, and T1 where a statute names it.** The birth-time field is never a hard gate on the paywall — charging for a reading the engine cannot compute (missing 시주) is a consumer-protection problem, so the 모름 path degrades to three pillars and names which conclusions are unavailable. The reading log keeps the question category and drops the free text: birth date and time are not per se 민감정보 under 개인정보 보호법 §23, but a log recording "asked about a health problem" or a 종교/신앙 framing creates 민감정보 by inference. PIPA guardian consent for under-14s — an app taking a birth date already knows when this applies. LLM-generated readings trigger the AI Basic Act Art. 31 advance-notice duty. Push consent and quiet hours under 정보통신망법 §50; no multiple daily pushes.
...
Do-not-quote corrections (no "Prevent Game Addiction Act", no "Japan 2025 gacha law", no "China 2025 random-draw rule"; Korea's complete-gacha bill 2212569 is pending in committee) live in `${CLAUDE_SKILL_DIR}/references/jurisdictions.md`.
