...
| §33-2 소송 특례 | 2025-08-01 | Reversed burden; ≤3× only if court finds intent |
...
- **출석 보상 is culturally load-bearing.** Daily attendance rewards are the default Korean retention primitive and are read as generous rather than manipulative — *unless* they punish absence (streak resets that destroy accumulated value). The distinction the skills should teach is reward-for-presence vs penalty-for-absence, which is also where PEGI's new interactive-risk criteria draw the line for games submitted from June 2026 (return rewards → PEGI 7; **punishing absence → PEGI 12**) (PEGI, announced 2026-03-12, applying to games newly submitted from June 2026; existing titles are not re-rated unless updated content warrants it — see 02f §2, http://www.pegi.info/index.php/news/pegi-expands-age-rating-criteria-interactive-risk-categories).
